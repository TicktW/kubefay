...
type Client interface {
	...
	InstallRemotePodFlows(podInfo string,
		peerPodIP net.IP,
		tunnelPeerIP net.IP,
		ipsecTunOFPort uint32) error

	InstallIPAMGatewayFlows(
		gatewayIP net.IP,
		nodeTunnelIP net.IP,
	) error
...
func (c *client) InstallRemotePodFlows(podInfo string,
	peerPodIP net.IP,
	tunnelPeerIP net.IP,
	ipsecTunOFPort uint32) error {
	c.replayMutex.RLock()
	...
	var flows []binding.Flow
	localGatewayMAC := c.nodeConfig.GatewayConfig.MAC
	if peerPodIP.To4() != nil {
		// Since broadcast is not supported in IPv6, ARP should happen only with IPv4 address, and ARP responder flows
		// only work for IPv4 addresses.
		flows = append(flows, c.arpResponderFlow(peerPodIP, cookie.Node))
	}
	if c.encapMode.NeedsEncapToPeer(tunnelPeerIP, c.nodeConfig.NodeIPAddr) {
		// tunnelPeerIP is the Node Internal Address. In a dual-stack setup, whether this address is an IPv4 address or an
		// IPv6 one is decided by the address family of Node Internal Address.
		flows = append(flows, c.l3FwdFlowToRemotePod(localGatewayMAC, peerPodIP, tunnelPeerIP, cookie.Node))
	}

	// return c.addFlows(c.nodeFlowCache, podInfo, flows)
	return c.ofEntryOperations.AddAll(flows)
}

func (c *client) InstallIPAMGatewayFlows(
	gatewayIP net.IP,
	nodeTunnelIP net.IP) error {
	c.replayMutex.RLock()
	defer c.replayMutex.RUnlock()
	...
	if gatewayIP.To4() != nil {
		...
		// for default gateway, only add l3 2 remote gateway
		flows = append(flows, c.l3FwdFlowToRemoteGateway(gatewayIP, nodeTunnelIP, cookie.Gateway))
		if gatewayIP.Equal(gatewayConfig.IPv4) {
			return c.ofEntryOperations.AddAll(flows)
		}

		flows = append(flows, c.arpIPAMGatewayFlow(gatewayIP, localGatewayMAC, cookie.Gateway))
		flows = append(flows, c.gatewayARPSpoofGuardFlow(gatewayIP, localGatewayMAC, cookie.Default))
