...
// NewIPAMConfig creates a NetworkConfig from the given network name.
func LoadIPAMConfigCRD(cniVersion string, subnet *v1alpha1.SubNet) (*IPAMConfig, string, error) {

	_, parseNet, err := net.ParseCIDR(subnet.Spec.CIDR)
	if err != nil {
	...
	for i := range IPAMconf.Ranges {
		if err := IPAMconf.Ranges[i].Canonicalize(); err != nil {
			return nil, "", fmt.Errorf("invalid range set %d: %s", i, err)
		}

		if IPAMconf.Ranges[i][0].RangeStart.To4() != nil {
			numV4++
		} else {
			numV6++
		}
	...
	if numV4 > 1 || numV6 > 1 {
		for _, v := range types020.SupportedVersions {
			if cniVersion == v {
				return nil, "", fmt.Errorf("CNI version %v does not support more than 1 address per family", cniVersion)
			}
		}
	}

	// Check for overlaps
	l := len(IPAMconf.Ranges)
	for i, p1 := range IPAMconf.Ranges[:l-1] {
		for j, p2 := range IPAMconf.Ranges[i+1:] {
			if p1.Overlaps(&p2) {
				return nil, "", fmt.Errorf("range set %d overlaps with %d", i, (i + j + 1))
			}
		}
	...
	return &IPAMconf, cniVersion, nil
}