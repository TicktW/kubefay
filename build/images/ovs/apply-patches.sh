...
# of OVS than the one kubefay is using) to OVS before building it. It needs to be
...
if version_get "$OVS_VERSION" "2.13.0" && version_lt "$OVS_VERSION" "2.14.0" ; then
    ...
    # ovs-vswitchd exit by default. kubefay relies on this to support hitless upgrade
    ...
    # Until the patch which fixes the script is merged upstream, we apply it here, or kubefay IPsec support will be broken.
...
# See https://github.com/kubefay-io/kubefay/issues/2003
